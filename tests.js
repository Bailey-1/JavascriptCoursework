...
test(`Characters:
  ...
  character codes.  e.g. 'A'=65, 'AB'=131 (i.e. 65+66), etc.`, function(assert) {
	...
	assert.equal(
		sumOfCharacters('PORT.AC.UK'),
		709,
		'The sum for PORT.AC.UK is 709'
	);
	assert.equal(
		sumOfCharacters('port.ac.uk'),
		965,
		'The sum for port.ac.uk is 965, a different score because they are different characters:-)'
	);
});
...
test(`Words:
  ...
  return an array containing the sum of the characters for each word.`, function(assert) {
...
test(`Sentence Sums:
  ...
  than the second parameter should be ignored.`, function(assert) {
...
test(`Palindrome:
  ...
  palindromes.`, function(assert) {
	...
	assert.equal(palindrome('A'), true, 'Single characters are palindromes');
...
test(`Emojify:
  ...
  should convert them to  ™️, ❤️, and 😀, respectively.`, function(assert) {
	...
	assert.equal(
		emojify(':-):-):-)<3<3<3'),
		'😀😀😀❤️❤️❤️',
		'Yay! 😀😀😀❤️❤️❤️ Even repeat emoji are replaced!'
	);
});
...
test(`Page Emojify:
  ...
  replace any shortcuts with their respective emoji.`, function(assert) {
...
test(`Tree Emojify:
  ...
  The element structure of the document must not change.`, function(assert) {
	...
	assert.equal(
		window.ea3 && window.ea3.textContent,
		'SoftEng™️ :-)',
		'Spans are honoured.'
	);
	assert.equal(
		window.ea4 && window.ea4.textContent,
		'Do you ❤️ me now?',
		'Heart works!'
	);
	assert.equal(
		window.ea5 && window.ea5.textContent,
		'😀😀😀❤️❤️❤️',
		'All smiles and hearts!'
	);
});
...
test(`Click Attacher:
  ...
  classname on the clicked element.`, function(assert) {
...
test(`Write a drawSaltire function that accepts one
   ...
   draw the Scottish flag to fit the canvas.`, assert => {
	...
	assert.ok(
		studentHappy,
		"This test passes when you decide it's finished (by returning true from your function)."
	);
});

test(`Write a drawUnion function that accepts one
   parameter 'elem' which is a canvas.  It should
...
test(`Advanced Challenge:
  ...
  achieve this challenge... it is not trivial!
   ...
   `, assert => {
	...
	assert.ok(false, "This test passes when you decide it's finished.");
});
